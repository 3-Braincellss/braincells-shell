...
class CdApp(App):
    ...
    def run(self, inp, out):
        ...
        If no arguments are given, the current directory is changed to the users **HOME** directory.
        ...
        Returns:
            ``deque``: Since the cd application does not return anything, output will be unaltered.
    ...
    def validate_args(self):
        ...
        Raises:
            ContextError: If more than one argument is supplied or the path given does
                not exist.
        """
        if len(self.args) > 1:

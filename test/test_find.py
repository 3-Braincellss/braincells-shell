...
class TestFind(ShellTestCase):
    ...
    def test_find_from_current_dir(self):
        ...
        app = FindApp(["-name", "f*"])
        app.run(None, out)
        self.assertEqual(set(["./dir_files/file-1", "./dir_files/file-2",
                         "./dir_files/file-3", "./dir_files/file-4", "./dir_files/file-5"]), set(out))
    ...
    def test_find_from_dir(self):
        ...
        app.run(None, out)
        self.assertEqual(
            set(["dir_nested/dir_nested.txt", "dir_nested/nest_1/dir_nested_1.txt", "dir_nested/nest_2/dir_nested_2.txt"]), set(out))
